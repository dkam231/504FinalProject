...
from combined.datasets.suim_binary_dataset import create_suim_binary_dataloaders
...
from combined.utils.checkpointing import save_checkpoint
from combined.utils.seed import seed_everything

def parse_args():
    parser = argparse.ArgumentParser(description="Train binary U-Net on SUIM.")
    parser.add_argument("--data-root", required=True, help="Path to SUIM root directory.")
    parser.add_argument("--output-dir", default="combined/outputs/suim")
...
def main():
    ...
    train_loader, val_loader = create_suim_binary_dataloaders(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        save_checkpoint(ckpt_dir / "suim_fg_bg_last.pth", model=model, optimizer=optimizer, epoch=epoch, metrics=val_metrics)

        if val_metrics["iou"] > best_iou:
            best_iou = val_metrics["iou"]
            save_checkpoint(ckpt_dir / "suim_fg_bg_best.pth", model=model, optimizer=optimizer, epoch=epoch, metrics=val_metrics)
            print("Saved best SUIM checkpoint.")
